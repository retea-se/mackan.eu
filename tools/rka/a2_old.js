...
function initTabs() {
  ...
  tabLinks.forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      // Update hidden field and trigger form submission
      if (activeTabField) {
        activeTabField.value = targetId;
        deb(); // Auto-submit when switching tabs
      }
    });
  });
}
...
document.getElementById('exportTXT').addEventListener('click', function() {
  // Hämta rapporten som text
  const html = getReportHtml();
  // Ta bort HTML-taggar och gör snygg text
  const temp = document.createElement('div');
  temp.innerHTML = html;
  let text = temp.innerText || temp.textContent || "";
  // Snygga till radbrytningar
  text = text.replace(/\n{3,}/g, '\n\n');
...
// Add auto-submit for "Aktuell last" fields
const aktuellIds = ['aktuellLast', 'aktuellTid'];
const aktuellSelIds = ['aktuellLastUnit'];

aktuellIds.forEach(id => {
  const el = document.getElementById(id);
  el && el.addEventListener('input', deb);
});
aktuellSelIds.forEach(id => {
  const el = document.getElementById(id);
  el && el.addEventListener('change', deb);
});
