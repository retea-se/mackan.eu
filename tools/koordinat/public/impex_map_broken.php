<?php
// tools/koordinat/public/impex_map_safe.php - Säker version med felhantering
error_reporting(E_ALL);
ini_set('display_errors', 1);

try {
    // Försök hitta layout-start på olika sätt
    $layoutPaths = [
        '../../../includes/layout-start.php',
        '../../includes/layout-start.php', 
        dirname(__DIR__, 3) . '/includes/layout-start.php'
    ];
    ...
    if (!$layoutFound) {
        // Fallback till enkel HTML om layout inte hittas
        echo '<!DOCTYPE html><html><head><title>GeoParser & Plotter</title><link rel="stylesheet" href="/css/main.css"></head><body>';
        echo '<p style="color: red;">Varning: Layout-fil inte hittad. Kör i fallback-läge.</p>';
    } else {
        // Sätt variablerna som layout-start förväntar sig
        $title = 'GeoParser & Plotter - Koordinatverktyg';
        $metaDescription = 'Avancerat verktyg för att parsa och visualisera geografiska koordinater från text.';
        $metaKeywords = 'koordinatparsning, geoparser, kartvisualisering, GPS-koordinater';
        ...
        $extraHead = '
            ...
            <style>
                #map { height: 500px; width: 100%; margin-top: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2); }
                .info-text { padding: 1rem; font-family: monospace; font-size: 1rem; text-align: right; color: #333; font-weight: bold; }
                .card .table th:last-child, .card .table td:last-child { white-space: nowrap; }
            </style>
        ';
    ...
    // Om allt misslyckas, använd minimal HTML
    echo '<!DOCTYPE html><html><head><title>GeoParser & Plotter - Fel</title></head><body>';
    echo '<h1>Systemfel</h1><p>Kunde inte ladda sidan: ' . htmlspecialchars($e->getMessage()) . '</p>';
    echo '<p><a href="/tools/koordinat/">Tillbaka till koordinatverktyget</a></p></body></html>';
    exit;
}
?>
...
<script src="impex_map.js"></script>

<?php
try {
    // Försök hitta layout-end
    $layoutEndPaths = [
        '../../../includes/layout-end.php',
        '../../includes/layout-end.php',
        dirname(__DIR__, 3) . '/includes/layout-end.php'
    ];
    
    $layoutEndFound = false;
    foreach ($layoutEndPaths as $path) {
        if (file_exists($path)) {
            include $path;
            $layoutEndFound = true;
            break;
