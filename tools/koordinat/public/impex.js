/**
 * impex.js - Version 36 (Fixad event-hantering + synlighet av funktioner)
 *
 * Funktionalitet:
...
console.log("[DEBUG] Laddar impex.js - Version 36");
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (exportButton) exportButton.addEventListener("click", () => exportTable("csv"));
});
...
function displayResults(results) {
    ...
    results.forEach(data => {
        let lat = data.wgs84?.lat ?? "undefined"; // Korrekt nyckel
        let lon = data.wgs84?.lon ?? "undefined"; // Korrekt nyckel
        let elevation = data.elevation ?? "N/A";
        let sweref99Zone = data.sweref99_zone ?? "N/A";

        let row = document.createElement("tr");
        row.innerHTML = `<td>${lat}</td><td>${lon}</td><td>${elevation} m</td><td>${sweref99Zone}</td>`;
